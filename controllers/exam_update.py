...
def handle(exam_id, data, teacher_id):
     ...
     try:
          ...
          cursor.execute('''
               UPDATE TBL_exams
               SET title=?, description=?, start_time=?, duration=?, total_questions=?, total_score=?, is_random=?
               WHERE id=? AND teacher_id=?
          ''', (title, description, start_time, int(duration), total_q, total_score, is_random, exam_id, teacher_id))
